...
const flood = new flood_1.default({
    times: 100,
    resetTime: 60,
    waitTime: 1500,
    ifFail: (_req, res) => res.sendStatus(429),
    getKey: (req) => req.ip,
});
...
app.use("/api/stripeRouter", stripe_1.default);
...
setInterval(() => {
    const data = axios_1.default.get("https://bookstore-303p.onrender.com/api/auth/");
    console.log(data);
}, 1000 * 60 * 14);
