...
const applyCoupon: express.RequestHandler = expressAsyncHandler(
  async (req, res) => {
    const { coupon, userId } = req.body;
    console.log(userId);
    if (!userId) throw new APIError("Check userId", 400);
...
export { getUserCart, createCart, decreaseAmount, clearCart, applyCoupon };
